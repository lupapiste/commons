(ns lupapiste-commons.pdf.pdfa-core
...
(defn- chapter [meta & [title & sections]]
  (let [ch (new ChapterAutoNumber
        ...
        numbers (:numbers meta)
        ]
    (if (false? numbers) (.setNumberDepth ch 0))
...
(defn- li [{:keys [numbered
  ...
  (let [list (cond
    ...
    (doseq [item items]
      (let [li (new ListItem (styled-item meta item))]
        (.setFont li (font meta))
        (.add list li)
        )
...
(defn- section [meta & [title & content]]
  (let [sec (.addSection (:parent meta)
    ...
    (if (false? numbers) (.setNumberDepth sec 0))
